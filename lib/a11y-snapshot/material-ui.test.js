...
describe.each(["chromium", "firefox"])("%s", (browserType) => {
	 ...
	 */
	let _onlyTouchInTestHooksBrowser;
	...
	beforeAll(async () => {
		_onlyTouchInTestHooksBrowser = await playwright[browserType].launch();
		context = await _onlyTouchInTestHooksBrowser.newContext();
		context.on("page", (page) => {
			// Debugging because sometimes jest seems to run a test before we closed all pages.
	...
	afterAll(async () => {
		await _onlyTouchInTestHooksBrowser.close();
	});

	afterEach(async () => {
		// can't run page.close in afterEach since tests are interleaved
		const pagesToClose = pages.slice();
		pages = [];
		await Promise.all(
			...
			["https://platform.twitter.com/**", "https://buttons.github.io/**"].map(
				(url) => {
					return page.route(url, (route) => {
						route.abort().catch(() => {
							console.log("error aborting %s", url);
						});
					});
				}
			)
	...
	it.each([
		...
		page
			.on("crash", () => {
				console.log("crash %s", docsRoute);
			})
			.on("pageerror", () => {
				console.log("pageerror %s", docsRoute);
			});
		await Promise.all(
			[
				"https://codefund.io/**",
				...
				return page.route(url, (route) => {
					route.abort().catch(() => {
						console.log("error aborting route %s", url);
					});
				});
			})
		);
