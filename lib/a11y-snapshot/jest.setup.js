...
const { extendExpect } = require("screen-reader-testing-library");
					...
					nodes: violation.nodes.flatMap((node) => {
						return node.target.map((selector) => {
							return selector
								...
								.replace(/\.css-\w+/g, ".css-REMOVED_UNSTABLE_EMOTION_HASH")
								.replace(/\.jss\d+/g, ".jssREMOVED_UNSTABLE_JSS_COUNTER");
						});
					}),
				};
			});
...
extendExpect(expect, __NVDA_LOG_FILE_PATH__);
