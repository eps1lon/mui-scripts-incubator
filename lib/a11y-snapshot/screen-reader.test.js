...
describe("nvda", () => {
	...
	test("settings drawer", async () => {
		...
		await expect(async () => {
			...
			`"clickable, banner landmark, Toggle settings drawer, button"`
		);
		...
		await expect(async () => {
			await page.keyboard.press("Tab");
					...
					"clickable, button"
					"Material-UI: A popular React UI framework - Chromium"
					"tool bar"
					"Address and search bar, edit, has auto complete, Ctrl+L, selected https://deploy-preview-25782--material-ui.netlify.app/?nvda-reset=l091i0zh28"
					"Material-UI: A popular React UI framework"
					"clickable, link, Skip to content"
					"banner landmark, button, Open main navigation"
					"clickable, Search, combo box, collapsed, has auto complete, editable, Search…"
					"Ctrl+K"
					"menu button, subMenu, Change language"
					"button, Toggle settings drawer"
					"menu button, subMenu, Toggle notifications panel"
					"link, GitHub repository"
					"main landmark, heading, level 1, MATERIAL-UI"
					"Material-UI is a simple and customizable component library to build faste"
				`);
