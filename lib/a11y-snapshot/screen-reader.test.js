...
describe("nvda", () => {
	...
	describe("TimePicker", () => {
		test("keyboard navigation for desktop variant", async () => {
			...
			).toMatchSpeechInlineSnapshot(`"clickable, Choose tim, button"`);

			await expect(() => page.keyboard.press("Enter"))
				.toMatchSpeechInlineSnapshot(`
						"dialog"
						"dialog"
						"Select hours. Selected time is 1:00 AM, list"
					`);

			await expect(() =>
				page.keyboard.press("ArrowUp")
			).toMatchSpeechInlineSnapshot(`"Select hours. Selected time is 2:00 AM"`);
			...
			await expect(() =>
				...
				`"clickable, main landmark, button, Choose tim"`
			);
		});
	});
});
