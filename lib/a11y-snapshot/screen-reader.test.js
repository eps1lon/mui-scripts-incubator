...
describe("nvda", () => {
	...
	describe("TimePicker", () => {
		test("keyboard navigation for desktop variant", async () => {
			...
			await expect(() => page.keyboard.press("Escape"))
				.toMatchSpeechInlineSnapshot(`
						"clickable, main landmark, button, A generic container that is programmatically focused to test keyboard navigation of our components."
						"clickable, Choose time, selected time is 12:00 AM, button"
					`);
		});
	});
});
